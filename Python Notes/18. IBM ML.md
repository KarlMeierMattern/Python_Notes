...
- Useful when we have a skewed distribution of out outcome variables and we want to maintain the skew.  
...
- Multinomial/multiclass logistic regression means the dependent variable has more than two categories i.e. not binary.  
...
- Binary classification problems or multi-class/multinomial classifaction problems can be treated as regression problems using encoding.  
- We can extend a binary classifation problem to a multinomial problem using a one-vs-all (OvA) technique, where each class you fit a binary classifier to that class versus the other classes.  
- The output of a logistic regression model applied to a sample is the probability of the sample being in a certain class.  
- Uses Sigmoid function y = 1/(1+e^-x).  
- Smooths out the impact of high or low values of x.  
- Y-values take on values between 0 and 1.  
- Decision boundary used to classify data.  
...
### Multinomial logistic regression vs OvA binary logistic regression  
...
- **0-1 loss function**: returns 1 for incorrectly classified records. Not used in practice as difficult to optimise (non-smooth and non-convex).  

### AdaBoost  
- Adaptive boosting.  
- Uses exponential loss function where loss = e^(-margin).  
- Makes AdaBoost more sensitive to outliers.  
