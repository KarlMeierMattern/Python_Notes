...
> It's prone to finding local minima: parameter estimates that aren't the best solution, but the gradient is zero.  
> This instability usually occurs when the step size or learning rate—the amount that each parameter is adjusted by each iteration—is too large.   
...
- Scikit learn classifiers won't accept strings for the prediction column - use `LabelEncoder` to convert labels to integers.  
- We can extend a binary classification problem to a multinomial problem using a one-vs-all (OvA) technique, where each class you fit a binary classifier to that class versus the other classes.  
